...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-15223';
...
values
('NULL', 'ProgramDetail', 'GoalsAndObjectives','Update1'),
('NULL', 'Program', 'Description','Update2'),
('NULL', 'ProgramQueryText', 'QueryTextId_03','Update3'),
('NULL', 'ProgramDetail', 'MasterPlanning','Update4'),
('NULL', 'ProgramQueryText', 'QueryTextId_04','Update5'),
('NULL', 'ProgramDetail', 'ProgramAlignment','Update6'),
('NULL', 'ProgramDetail', 'SimilarPrograms','Update7')
...
SELECT 0 AS Value, Concat('<p>Upon successful completion of this program, students will be able to:</p>',
dbo.ConcatWithSepOrdered_Agg('<br>', po.Sortorder, po.Outcome)
) AS Text
...
'<b>Program Control Number</b>: ', p.UniqueCode2
) AS Text
FROM Program AS p
...
VALUES
(@MAX, 'Program', 'Id', 'Title', @SQL, @SQL, 'Order By SortOrder', 'Program Outcomes Query', 2),
(@MAX2, 'Program', 'Id', 'Title', @SQL3, @SQL3, 'Order By SortOrder', 'Program Codes and Dates Query', 2)
...
WHERE MetaSelectedFieldId in (
	SELECT FieldId FROM @Fields WHERE Action not in (
		'Update1'
	)
)
...
SELECT DISTINCT
'Transfer Preparation', -- [DisplayName]
2304, -- [MetaAvailableFieldId]
...
UNION
SELECT DISTINCT
'Major Number', -- [DisplayName]
1640, -- [MetaAvailableFieldId]
SectionId, -- [MetaSelectedSectionId]
0, -- [IsRequired]
NULL, -- [MinCharacters]
NULL, -- [MaxCharacters]
...
exec upGenerateGroupConditionsCourseBlockDisplay @entityId = @entityId, @config = @config;
'
