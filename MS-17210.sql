...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-17210';
...
--WHERE MetaAvailableFieldId in (2916, 2917)

--SELECT * FROM Client

--SELECT * FROM MetaAvailableField WHERE MetaAvailableFieldId in (2916, 2917)
...
UPDATE ProgramYesNo
SET YesNo03Id = CASE WHEN p.PickListYes_No01Id = 97 THEN 1 WHEN p.PickListYes_No01Id = 98 THEN 2 WHEN p.PickListYes_No01Id = 276 THEN 3 ELSE NULL END
, YesNo02Id = CASE WHEN p.PickListYes_No02Id = 202 THEN 1 WHEN p.PickListYes_No02Id = 203 THEN 2 WHEN p.PickListYes_No02Id = 277 THEN 3 ELSE NULL END
FROM ProgramYesNo AS pyn
INNER JOIN Program AS pp on pyn.ProgramId = pp.Id
INNER JOIN PickListOneToOne as p on p.ProgramId = pp.Id
WHERE p.ProgramId IS NOT NULL
