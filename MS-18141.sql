...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-18141';
DECLARE @Comments nvarchar(Max) = 
	'Move Textbox and change its label';
...
values
('Cover', 'GenericMaxText', 'TextMax08','Move'),
('Cover', 'GenericBit', 'Bit06','Move1')
...
/********************** Changes go HERE **************************************************/
UPDATE mds
SET MetaSelectedSectionId = NULL
, MetaSelectedFieldId = f.FieldId
FROM MetaDisplaySubscriber AS mds
INNER JOIN @Fields AS f on mds.MetaSelectedSectionId = f.SectionId
WHERE f.Action = 'Move'

UPDATE MetaSelectedField
SET RowPosition = RowPosition + 1
WHERE MetaSelectedFieldId in (
	SELECT FieldId FROM @Fields WHERE Action = 'Move1'
)
