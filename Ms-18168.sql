...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-18168';
DECLARE @Comments nvarchar(Max) = 
	'Update Entry Skills tab and forms';
...
values
('Entry Skills', 'CourseEntrySkill', 'MaxText01','1'),
('Entry Skills', 'CourseEntrySkillCourseObjective', 'Requisite_CourseId','2')
...
WHERE MetaSelectedFieldId in (
	...
	WHERE MetaSelectedSectionId in (
		SELECT SectionId FROM @Fields WHERE mtt in (10, 15)
	)
)
...
WHERE MetaSelectedSectionId in (
	SELECT SectionId FROM @Fields WHERE mtt in (10, 15)
		UNION
	SELECT TabId FROM @Fields WHERE mtt in (10, 15)
)
...
SET DisplayName = 'Requisite Course Objective'
WHERE MetaForeignKeyLookupSourceId = 110
/****************************** update templates ******************************************/
update MetaTemplate
...
where MetaTemplateId in (
	...
	WHERE msf.MetaForeignKeyLookupSourceId in (122, 110)
)
