...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-18925';
DECLARE @Comments nvarchar(Max) = 
	'Update Query for checklist';
...
SELECT DISTINCT 
    od.Id AS Value, 
    oe.Title AS Text, 
    Parent_OrganizationEntityId AS filterValue
FROM OrgData od
...
WHERE od.OrganizationTierId = 2
    ...
    AND od.Id NOT IN (
        1249, 1250, 1251, 1253, 1254, 1255, 1256, 1257, 1260, 1261, 1263, 1264, 1265, 1266, 1268, 1290
    )
UNION
SELECT 
    oe.Id AS Value, 
...
FROM OrganizationEntity AS oe 
WHERE Id = 47 
AND 1231 IN (SELECT Id FROM @tier1) -- This ensures the row is only included if 1231 is in @tier1
ORDER BY Text;
'
