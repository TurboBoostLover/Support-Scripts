...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-13660';
DECLARE @Comments nvarchar(Max) = 
	'Update Articulations Report';
...
WHERE mt.Active = 1 
	...
	AND mtt.MetaTemplateTypeId = 8 -- hard code
...
values
('Program Contact Information', 'ProgramEntityContact', 'ContactName','Delete')
...
DECLARE @Mtid int = (SELECT TemplateId FROM @Fields)
DECLARE @Tabid int = (SELECT TabId FROM @Fields)
DECLARE @RmId int = (SELECT SectionId FROM @Fields)

EXEC spBuilderSectionDelete @clientId, @RmId
...
DECLARE @SecId int = SCOPE_IDENTITY()
...
1, -- [AllowCopy]
NULL, -- [Precision]
73, -- [MetaForeignKeyLookupSourceId]
NULL, -- [MetadataAttributeMapId]
NULL, -- [EditMapId]
