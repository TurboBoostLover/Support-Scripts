USE [laspositas];

/*
   Commit
									Rollback
*/

DECLARE @JiraTicketNumber nvarchar(20) = 'DST-5304';
DECLARE @Comments nvarchar(Max) = 
	'';
DECLARE @Developer nvarchar(50) = 'Nathan Westergard';
DECLARE @ScriptTypeId int = 1; /*  Default 1 is Support,  
For a complete list run the following query

Select * from history.ScriptType
*/

SELECT
 @@servername AS 'Server Name' 
,DB_NAME() AS 'Database Name'
,@JiraTicketNumber as 'Jira Ticket Number';

SET XACT_ABORT ON
BEGIN TRAN

INSERT INTO History.ScriptsRunOnDatabase
(TicketNumber,Developer,Comments,ScriptTypeId)
VALUES
(@JiraTicketNumber, @Developer, @Comments, @ScriptTypeId); 

/*--------------------------------------------------------------------
Please do not alter the script above this comment  except to set
the Use statement and the variables. 

Notes:  
	1.   In comments put a brief description of what the script does.
         You can also use this to document if we are doing somehting 
		 that is against meta best practices but the client is 
		 insisting on, and that the client has been made aware of 
		 the potential consequences
	2.   ScriptTypeId
		 Note:  For Pre and Post Deploy we should follow the following 
		 script naming convention Release Number/Ticket Number/either the word Predeploy or PostDeploy
		 Example: Release3.103.0_DST-4645_PostDeploy.sql

-----------------Script details go below this line------------------*/
DECLARE @SQL NVARCHAR(MAX) = '
declare @entryCount int = (
    select count(*)
    from CoursePeriodical cp
    where Cp.CourseId = @entityId
);

declare @entryCount2 int = (
    select count(*)
    from CourseTextbook cp
    where Cp.CourseId = @entityId
);

declare @entryCount3 int = (
    select count(*)
    from CourseManual cp
    where Cp.CourseId = @entityId
);

declare @entryCount4 int = (
    select count(*)
    from CourseSoftware cp
    where Cp.CourseId = @entityId
);

declare @entryCount5 int = (
    select count(*)
    from CourseTextOther cp
    where Cp.CourseId = @entityId
);

declare @validCount int = 1

declare @IsValidCount bit = (select cast(
case
	when @entryCount >= @validCount 
	then 1
	when @entryCount2 >= @validCount
	THEN 1
	when @entryCount3 >= @validCount 
	then 1 
	when @entryCount4 >= @validCount
	THEN 1
	when @entryCount5 >= @validCount 
	then 1 
	else 0 end as bit))

	SELECT yn.Id AS Value,
	CASE WHEN @IsValidCount = 1
	THEN yn.Title
	ELSE NULL
	END AS Text
	FROM YesNo AS yn 
	WHERE yn.Title = ''Yes''
'

INSERT INTO MetaSqlStatement
(SqlStatement, SqlStatementTypeId)
VALUES
(@SQL, 1)

DECLARE @ID int = SCOPE_IDENTITY()

INSERT INTO MetaControlAttribute
(MetaSelectedSectionId, Name, Description, MetaControlAttributeTypeId, MetaAttributeComparisonTypeId, CustomMessage, MetaSqlStatementId)
VALUES
()