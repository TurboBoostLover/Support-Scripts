...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-14353';
...
values
('Textbook', 'CourseYesNo', 'YesNo04Id','Ping')
...
DECLARE @CSQL NVARCHAR(MAX) = "
	SELECT 0 AS Value, '<a href=""https://sjcc.edu/on-campus-resources/library/oer.aspx"" target=""_blank"">More information on OER.</a>' AS Text
"
...
VALUES
(@MAX, 'CourseQueryText', 'Id', 'Title', @CSQL, @CSQL, 'Order By SortOrder', 'HyperLink', 1)

UPDATE MetaSelectedSection
SET SectionName = 'Textbook'
WHERE MetaSelectedSectionId in (
	SELECT TabId FROM @Fields WHERE Action = 'Ping'		--Bad data with trailing space
)
...
AND RowPosition = 0
AND DefaultDisplayType = 'Hyperlink'
/****************************** update templates ******************************************/
update MetaTemplate
set LastUpdatedDate = getdate()
where MetaTemplateId in (select Distinct templateId FROM @Fields)

exec EntityExpand @clientId =49 , @entityTypeId =1
