...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-15438';
DECLARE @Comments nvarchar(Max) = 
	'Update Admin Unit Program Review';
...
values
('Cover', 'ModuleSystemData', 'Tier2_OrganizationEntityId','Update')
...
VALUES
(@MAX, 'OrganizationEntity', 'Id', 'Title', @CSQL, @RSQL, 'Order By SortOrder', 'Looks up permission for Program Checklist', 2)
...
SELECT 'lookuptablename','ModuleEvaluationMethod', @SECTION
UNION
SELECT 'lookupcolumnname','ProgramId',@SECTION
UNION
SELECT 'columns','1', @SECTION

INSERT INTO ModuleEvaluationMethod
(ModuleId, ProgramId)
SELECT msd.ModuleId, msd.Tier2_OrganizationEntityId fROM ModuleSystemData AS msd
INNER JOIN Module AS m on msd.ModuleId = m.Id
WHERE m.MetaTemplateId in (
	SELECT TemplateId FROM @Fields
)

INSERT INTO MetaControlAttribute
(MetaSelectedSectionId, Name, Description, MetaControlAttributeTypeId, MetaAttributeComparisonTypeId, TotalCount)
VALUES
(@SECTION, 'Program in Program Review', 'Must select at least one item in check list', 5, 6, 1)
/****************************** update templates ******************************************/
update MetaTemplate
