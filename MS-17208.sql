...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-17208';
DECLARE @Comments nvarchar(Max) = 
	'Add Custom Query for Policy Term';
...
WHERE mt.Active = 1 
    ...
    AND mtt.ClientId = @clientId
		AND mtt.MetaTemplateTypeId in (493, 494)		--comment back in if just doing some of the mtt's
...
values
('Policy', 'Module', 'EffectiveTerm_SemesterId','Update')
...
from [Semester]  
where active = 1   
and AcademicYearStart >= (year(GetDate()) -2)  
and AcademicYearEnd <= (year(GetDate()) +5)   
Union    
select s.[Id] as [Value], 
(s.Title) as [Text] ,
SortOrder  
from [Semester] s  
inner join Module m on m.EffectiveTerm_SemesterId = s.Id
Where m.Id = @EntityId
Order by SortOrder
"

DECLARE @RSQL NVARCHAR(MAX) = "
select (Title) as [Text]      from [Semester]      where Id = @Id 
"
...
VALUES
(@MAX, 'Semester', 'Id', 'Title', @CSQL, @RSQL, 'Order By SortOrder', 'EffectiveTerm_SemesterId', 2)
