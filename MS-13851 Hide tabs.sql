...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-13851';
DECLARE @Comments nvarchar(Max) = 
	'Hide Course Materials Tab on reports';
...
WHERE mt.Active = 1 
	...
	AND mtt.MetaTemplateTypeId NOT IN (14, 15, 23) --remove reports where not in them
...
values
--('Course Materials', 'GenericOrderedList01', 'Lookup14Id','Update'),
('Course Materials', 'CourseTextbook', 'Author', 'Update'),
('Course Materials', 'GenericOrderedList01', 'Text1000_01', 'Update'),
('Course Materials', 'CourseManual', 'Author', 'Update'),
('Course Materials', 'CourseSoftware', 'Title', 'Update'),
('Course Materials', 'CourseTextOther', 'ConditionId', 'Update')
...
--(Title, Active, ClientId, ProgramCode, FilterSubject, PositionTypeId, HasAgendaReport, CanEditInReview, IsGlobal, CanCensorComments, CrossListingAdmin, CrossListingNotification, CanAttachInReview)
--VALUES
--('Hide', 1, 1, NULL, 0, 3, 0, 0, 0, 0, 0, 0, 0)

--DECLARE @ID int = SCOPE_IDENTITY()

--INSERT INTO MetaSelectedSectionPositionPermission
--(MetaSelectedSectionId, PositionId, AccessRestrictionType)
--SELECT TabId, @ID, 1 FROM @Fields

INSERT INTO MetaSelectedSectionRolePermission
(MetaSelectedSectionId, RoleId, AccessRestrictionType)
SELECT DISTINCT TabId, 1, 1 FROM  @Fields
UNION
