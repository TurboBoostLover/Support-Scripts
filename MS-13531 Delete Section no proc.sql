...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-13531';
DECLARE @Comments nvarchar(Max) = 
	'Remove Section "Does this course meet any of the following characteristics? Select all that apply:" from course forms';
...
values
('Course Standards', 'CourseAdditionalResource', 'AdditionalResourceId','Remove')
...
/********************** Changes go HERE **************************************************/
DECLARE @row INTEGERS 

INSERT INTO @row
	SELECT RowPosition - 2 FROM MetaSelectedSection		--go up two rows for static text section
	WHERE MetaSelectedSectionId IN (Select SectionId FROM @Fields)

DECLARE @MSSID INTEGERS

INSERT INTO @MSSID
	SELECT MetaSelectedSectionId FROM MetaSelectedSection		--grab section id based of rows in a tab
	WHERE RowPosition in (SELECT * FROM @row)
	and MetaSelectedSection_MetaSelectedSectionId in (SELECT TabId FROM @Fields)

DELETE FROM MetaDisplaySubscriber
	WHERE MetaSelectedSectionId IN (SELECT * FROM @MSSID)
	OR MetaSelectedSectionId IN (Select SectionId FROM @Fields WHERE Action = 'Remove')
...
DELETE FROM MetaSelectedSection
	WHERE MetaSelectedSectionId IN (Select SectionId FROM @Fields WHERE Action = 'Remove')
