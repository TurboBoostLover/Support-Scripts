...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-14895';
DECLARE @Comments nvarchar(Max) = 
	'Enable splash screen and disable old search';
    ...
    },
    {
        ...
        "settings": [
            ...
            },
            {
                ...
                "Value": "ORDER BY pt.Active desc, pt.ClientId, pt.ProcessActionTypeId, pt.Title",
                "Active": true
            },
            {
                "AccessLevel": "curriqunet",
...
DECLARE @clientCount INT = (SELECT COUNT(*) FROM Client WHERE Active = 1);
DECLARE @isDistrict BIT = CASE WHEN COALESCE(@clientCount, 0) > 1 THEN 1 ELSE 0 END;

IF (@isDistrict = 0) 
BEGIN;

	-- public search
	MERGE INTO Config.PublicSearch t
	USING (
		SELECT
            cl.Id as ClientId, 
            lt.Id as OrganizationTierId, 
            NULL as ShowDetail, 
            1 as IncludeAwardType,
            1 AS IncludeOrganizationEntity,
            NULL as AwardTerm, 
			NULL as CreditTerm, 
			NULL as SubjectTerm,
			--Values for use in output
			cl.Title as ClientTitle, cl.Code as ClientCode
		FROM Client cl
			CROSS APPLY (
				--Grab the Id of the lowest tier that has subjects tied to it, this should be the
				--department (or equivalent) tier, which is most likely to be the tier users would
				--want to filter on
				select top 1 ot.Id
				from OrganizationTier ot
				where ot.ClientId = cl.Id
				and exists (
					select 1
					from OrganizationSubject os
					inner join OrganizationEntity oe on os.OrganizationEntityId = oe.Id
					where ot.Id = oe.OrganizationTierId
				)
				order by ot.SortOrder desc
			) lt
		WHERE cl.Active = 1
	) s ON (t.ClientId = s.ClientId)
		WHEN NOT MATCHED THEN 
			INSERT (ClientId, OrganizationTierId, ShowDetail, IncludeAwardType, IncludeOrganizationEntity, AwardTerm, CreditTerm, SubjectTerm)
			VALUES (s.ClientId, s.OrganizationTierId, s.ShowDetail, s.IncludeAwardType, s.IncludeOrganizationEntity, s.AwardTerm, s.CreditTerm, s.SubjectTerm);

	-- cet
	IF(NOT EXISTS (SELECT 1 FROM ClientEntityType cet WHERE COALESCE(cet.PublicSearchVisible, 0) = 1))
	BEGIN;
		UPDATE cet
		SET cet.PublicSearchVisible = 1
		FROM ClientEntityType cet
			inner join Client cl ON cet.ClientId = cl.Id
			inner join EntityType et ON cet.EntityTypeId = et.Id
		WHERE cl.Active = 1
		and cet.Active = 1
		and cet.EntityTypeId in (1, 2);
	END;

	-- public report
	IF(NOT EXISTS(SELECT 1 from MetaReport mr where JSON_VALUE(mr.ReportAttributes,'$.isPublicReport') = 'true'))
	BEGIN;
		UPDATE mr
		SET ReportAttributes = JSON_MODIFY(ReportAttributes,'$.isPublicReport','true')
		FROM MetaReport mr
			INNER JOIN MetaReportType mrt ON mr.MetaReportTypeId = mrt.Id
		WHERE mr.MetaReportTypeId IN (1, 5)
		AND EXISTS (
			SELECT 1
