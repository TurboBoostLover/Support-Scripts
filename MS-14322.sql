...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-14372';
...
WHERE mt.Active = 1 
    ...
    AND mtt.ClientId = @clientId
		AND mtt.MetaTemplateTypeId in (532)		--comment back in if just doing some of the mtt's
...
values
('Student Data Assessment / Analysis', 'ModuleCRNStudentData', 'StudentDataId','Update')
...
SET Value = '<span style="color:red; text-decoration: underline; font-weight: bold;">CLICK HERE - To select all students as meeting the SLO.'
WHERE MetaSelectedSectionId in (
	SELECT SectionId FROM @Fields WHERE Action = 'Update'
)
AND Name = 'paneldefaultname'
