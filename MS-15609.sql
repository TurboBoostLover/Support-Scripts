...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-15609';
DECLARE @Comments nvarchar(Max) = 
	'Add Co-Contributor section to courses';
...
values
('Codes/Dates', 'CourseYesNo', 'YesNo29Id','Ping')
...
DECLARE @ID int = (SELECT SubjectId FROM Course WHERE Id = @EntityId)

SELECT u.Id AS Value
, CONCAT(u.LastName, ', ', u.FirstName, ' (', u.Email, ')') AS Text
FROM [User] AS u
INNER JOIN UserOriginationSubjectPermission as us on us.UserId = u.Id
WHERE u.Active = 1
and us.Active = 1
and us.SubjectId = @ID
Order BY u.LastName
...
Select u.LastName + ', ' + u.FirstName as Text from [user] u where Id = @Id
"
...
VALUES
(@MAX, 'User', 'Id', 'Title', @CSQL, @RSQL, 'Order By SortOrder', 'Sorted co-contributor list', 2)

DECLARE @TABLE TABLE (SecId int)
...
output inserted.MetaSelectedSectionId INTO @Table
SELECT DISTINCT
1, -- [ClientId]
TabId, -- [MetaSelectedSection_MetaSelectedSectionId]
'Co-Contributor', -- [SectionName]
...
SELECT DISTINCT
'Co-Contributor', -- [DisplayName]
1384, -- [MetaAvailableFieldId]
