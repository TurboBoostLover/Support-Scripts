...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-15313';
DECLARE @Comments nvarchar(Max) = 
	'Update Program Review Module';
...
('Section III: Summary Report', 'ModuleCRN', 'TextMax02', 'Update2'),
('Section III: Summary Report', 'ModuleCRN', 'TextMax01', 'Update3')
...
select cast(case when @entryCount > 2 then 1 else 0 end as bit) as IsValidCount;', 1)
...
SELECT SectionId, 'Action Steps', 'Must put in 3', 6, 6, 'Please enter a minimum of three program objectives.', @ID FROM @Fields WHERE Action = 'Update'

UPDATE MetaControlAttribute
SET CustomMessage = 'Please enter at least one action step for each objective identified in section I.'
WHERE Id in (
16, 17
)
...
WHERE MetaSelectedFieldId in (
	SELECT FieldId FROM @Fields WHERE Action = 'Update2' and mtt = 42
)

UPDATE MetaSelectedField
SET IsRequired = 1
WHERE MetaSelectedFieldId in (
	SELECT FieldId FROM @Fields WHERE Action = 'Update3' and mtt = 41
)
/****************************** update templates ******************************************/
update MetaTemplate
