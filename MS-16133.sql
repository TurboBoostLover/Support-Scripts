...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-16133';
DECLARE @Comments nvarchar(Max) = 
	'Fix attachments in Program Review';
...
values
('Attachments', 'ModuleAttachedFile', 'Title','test'),
('CSLO Assessment', 'ModuleAttachedFile', 'Title','test2'),
('III. Students and Student Success', 'ModuleAttachedFile', 'Title', 'test3')
...
INNER JOIN MetaSelectedSection AS mss on f2.TabId = mss.MetaSelectedSectionId
WHERE f.Action = 'test'
and f2.Action = 'test2'
UNION
SELECT 'triggersectionrefresh', mss.MetaSelectedSection_MetaSelectedSectionId, f.SectionId FROM @Fields AS f 
INNER JOIN @Fields AS f2 on f.TemplateId = f2.TemplateId
INNER JOIN MetaSelectedSection AS mss on f2.TabId = mss.MetaSelectedSectionId
WHERE f.Action = 'test'
and f2.Action = 'test2'
...
WHERE f.Action = 'test2'
and f2.Action = 'test3'
UNION
SELECT 'triggersectionrefresh', mss.MetaSelectedSection_MetaSelectedSectionId, f2.SectionId FROM @Fields AS f 
INNER JOIN @Fields AS f2 on f.TemplateId = f2.TemplateId
INNER JOIN MetaSelectedSection AS mss on f.TabId = mss.MetaSelectedSectionId
WHERE f.Action = 'test2'
and f2.Action = 'test3'
