...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-16370';
...
values
('Section II - Action Steps', 'ModuleStrategicGoal', 'MaxText01','update'),
('Section II: Action Steps', 'ModuleStrategicGoal', 'MaxText01','update')
...
SELECT Id AS Value,
MaxText02 AS Text
FROM ModuleStrategicGoal 
WHERE ModuleId = @EntityId
"

DECLARE @RSQL NVARCHAR(MAX) = "
SELECT MaxText02 AS Text
FROM ModuleStrategicGoal 
WHERE Id = @Id
"
...
VALUES
(@MAX, 'ModuleStrategicGoal', 'Id', 'Title', @CSQL, @RSQL, 'Order By SortOrder', 'Objective', 3)

insert into [MetaSelectedField]
([DisplayName], [MetaAvailableFieldId], [MetaSelectedSectionId], [IsRequired], [MinCharacters], [MaxCharacters], [RowPosition], [ColPosition], [ColSpan], [DefaultDisplayType], [MetaPresentationTypeId], [Width], [WidthUnit], [Height], [HeightUnit], [AllowLabelWrap], [LabelHAlign], [LabelVAlign], [LabelStyleId], [LabelVisible], [FieldStyle], [EditDisplayOnly], [GroupName], [GroupNameDisplay], [FieldTypeId], [ValidationRuleId], [LiteralValue], [ReadOnly], [AllowCopy], [Precision], [MetaForeignKeyLookupSourceId], [MetadataAttributeMapId], [EditMapId], [NumericDataLength], [Config])
SELECT
'ModuleStrategicGoal', -- [DisplayName]
...
VALUES
('Action Step', 1, 'ModuleModuleObjective', 'ModuleStrategicGoalId', 1, GETDATE(), @clientId)

DECLARE @ID int = SCOPE_IDENTITY()

DELETE FROM ListItemType WHERE ID = 13

DELETE FROM ModuleModuleObjective WHERE ModuleId in (
	SELECT M.Id FROM Module AS m
	INNER JOIN @templateId AS t on m.MetaTemplateId = t.Id
)

DECLARE @TABLE TABLE (ModuleStrategicGoalId int, ActionStep nvarchar(max), ActionStepProgress nvarchar(max), ModuleId int)
INSERT INTO @TABLE
SELECT Id, MaxText02, MaxText03, ModuleId
FROM ModuleStrategicGoal
WHERE ModuleId in (
	SELECT M.Id FROM Module AS m
	INNER JOIN @templateId AS t on m.MetaTemplateId = t.Id
...
(ModuleId, ModuleStrategicGoalId, MaxText01, MaxText02)
SELECT ModuleId, ModuleStrategicGoalId, ActionStep, ActionStepProgress FROM @TABLE

DECLARE @SQL NVARCHAR(MAX) = '
	DECLARE @Objectives TABLE (txt nvarchar(max), ActionStep nvarchar(max), progress nvarchar(max))
	INSERT INTO @Objectives
	SELECT msg.MaxText01, mmo.MaxText01, mmo.MaxText02 FROM ModuleModuleObjective AS mmo
	INNER JOIN ModuleStrategicGoal AS msg on mmo.ModuleStrategicGoalId = msg.Id
	WHERE mmo.ModuleId = @EntityId

	DECLARE @Results TABLE (txt nvarchar(max))
	INSERT INTO @Results
	SELECT
	CONCAT (
	''<b>Objective: </b>'', o.txt,
	''<br>'',
	''<b>Action Step(s): </b>'', dbo.ConcatWithSep_Agg(''; '', o.ActionStep),
	''<br>''
	) AS Text
	...
	SELECT 0 AS Value,
	dbo.ConcatWithSep_Agg(''<br>'', txt) AS Text
	FROM @Results
'
