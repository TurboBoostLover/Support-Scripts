...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-17511';
DECLARE @Comments nvarchar(Max) = 
	'Update Request Faculty on the Instructional PRogram Review';
...
WHERE mt.Active = 1 
    ...
    AND mtt.ClientId = @clientId
		AND mtt.MetaTemplateTypeId in (36)		--comment back in if just doing some of the mtt's
...
values
('Faculty', 'ModuleResourceRequestLookup', 'Lookup01Id','look'),
('Faculty', 'ModuleResourceRequestLookup', 'ExplainLookup02', 'delete'),
('Faculty', 'ModuleResourceRequest', 'MaxText01', '1'),
('Faculty', 'ModuleResourceRequest', 'ShortText01', 'other'),
('Faculty', 'ModuleResourceRequest', 'Lookup04Id_02', 'move')
...
VALUES
(@MAX, 'Lookup01', 'Id', 'Title', @CSQL, @RSQL, 'Order By SortOrder', 'look up instructional', 1)

UPDATE MetaSelectedField
SET MetaForeignKeyLookupSourceId = @MAX
WHERE MetaSelectedFieldId in (
	SELECT FieldId FROM @Fields WHERE Action = 'look'
)

UPDATE MetaForeignKeyCriteriaClient
SET CustomSql = 'select [Id] as [Value], (ShortText) as [Text] 
from [Lookup04] 
where Active = 1 
...
INSERT INTO Lookup04
(ClientId, ShortText, SortOrder, StartDate)
VALUES
(1, 'Instructional Support', 3, GETDATE())
...
UPDATE MetaSelectedField
SET DisplayName = 'Explain how students/area will be adversely affected without this faculty position in the context of the Chaffey Goal(s) selected above.'
WHERE MetaSelectedFieldId in (
...
DECLARE @Trigger int = SCOPE_IDENTITY()
DECLARE @Listener int = (SELECT FieldId FROM @Fields WHERE Action = 'other')

exec upAddShowHideRule @Trigger, null, 2, 3, 4, 'false', null,
@Listener, null, 'Show or Hide Other on Faculity', 'Other'

UPDATE ModuleResourceRequest
SET Bit10 = 1
WHERE ShortText01 IS NOT NULL and LEN(ShortText01) > 1
/****************************** update templates ******************************************/
update MetaTemplate
