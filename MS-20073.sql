...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-20073';
DECLARE @Comments nvarchar(Max) = 
	'Update Program Deactivation so users can edit some fields';
...
WHERE mt.Active = 1 
    ...
    AND mtt.ClientId = @clientId
		AND mtt.MetaTemplateTypeId in (489)		--comment back in if just doing some of the mtt's
...
values
('Main', 'ProgramDetail', 'GoalsAndObjectives','1'),
('Completed by Dean', 'ProgramDate', 'ProgramDateTypeId', '2')
...
WHERE MetaSelectedSectionId in (
	SELECT SectionId FROM @Fields WHERE Action in ('1', '2')
)
