...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-16279';
DECLARE @Comments nvarchar(Max) = 
	'Fix Attributes for cloneing';
...
values
('Codes and Dates', 'CourseDate', 'CourseDateTypeId','Ping')
...
BEGIN
    ...
    DECLARE @Sec INT = (SELECT TOP 1 Id FROM @Sections)
    ...
    VALUES
    ('DuplicationOverride BlacklistDoNotClone', 'BlacklistDoNotClone', 20, @Id, 'Text')

    UPDATE MetaSelectedSection
    SET MetadataAttributeMapId = @Id
    WHERE MetaSelectedSectionId = @Sec

    DELETE FROM @Sections WHERE Id = @Sec
END
