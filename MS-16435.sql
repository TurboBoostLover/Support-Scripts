...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-16435';
DECLARE @Comments nvarchar(Max) = 
	'Update Admin report Package time to completion';
...
UPDATE AdminReport
SET ReportName = 'proposal time to completion'
, ReportSQL = '
...
SELECT
    ...
    pr.ImplementDate AS [Implement Date],
		CASE 
			WHEN LEN(entity.Description) > 32000
			THEN ''To Long for Excel''
			ELSE
    entity.Description
		END AS [Description]
FROM (
    SELECT p.Id, p.Title, p.CreatedDate, p.Description, p.StatusAliasId, p.ProposalId, p.ProposalTypeId, ''Package'' AS EntityType FROM Package AS p
    UNION ALL
    SELECT c.Id, c.Title, c.CreatedDate, c.Description, c.StatusAliasId, c.ProposalId, c.ProposalTypeId, ''Course'' AS EntityType FROM Course AS c
    UNION ALL
    SELECT prg.Id, prg.Title, prg.CreatedDate, prg.Description, prg.StatusAliasId, prg.ProposalId, prg.ProposalTypeId, ''Program'' AS EntityType FROM Program AS prg
    UNION ALL
    SELECT m.Id, m.Title, m.CreatedDate, m.Description, m.StatusAliasId, m.ProposalId, m.ProposalTypeId, ''Module'' AS EntityType FROM Module AS m
) AS entity
...
and entity.StatusAliasId in (628, 633)
ORDER BY entity.Title;
'
WHERE Id = 11

UPDATE AdminReportFilter
SET FilterLabel = 'Created Date after'
WHERE Id = 7