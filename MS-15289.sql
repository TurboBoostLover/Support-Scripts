...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-15289';
DECLARE @Comments nvarchar(Max) = 
	'change show hide on CTE tab';
...
values
('Basic Program Information', 'Program', 'CampusId','Trigger'),
('CTE Documentation', 'GenericBit', 'Bit08', 'Listener'),
('CTE Documentation', 'GenericBit', 'Bit11', 'Listener2'),
('CTE Documentation', 'GenericBit', 'Bit13', 'Listener3')
...
SELECT f.TemplateID, f.FieldId, f2.FieldId FROM @Fields AS F
INNER JOIN @Fields AS F2 on f.TemplateId = f2.TemplateId
 WHERE f.Action = 'Trigger'
 and f2.Action = 'Listener'

	declare @Template int = (SELECT TOP 1 TId FROM @SHOWHIDE WHERE TId IS NOT NULL)
	declare @Trigger int = (SELECT TOP 1 field FROM @SHOWHIDE WHERE field IS NOT NULL and TId = @Template)
	declare @Sec int = (SELECT TOP 1 sec FROM @SHOWHIDE WHERE sec IS NOT NULL and TId = @Template)
	...
	DECLARE @DisplayRuleName NVARCHAR(50) = 'Show/hide Minutes';    
	DECLARE @SubscriberName NVARCHAR(50) = 'Show/hide Minutes';    
	...
	(SubscriberName, MetaSelectedFieldId, MetaSelectedSectionId, MetaDisplayRuleId)
	SELECT @SubscriberName, FieldId, NULL, @displayRuleId FROM @Fields WHERE ACtion in (
	'Listener2', 'Listener3'
	)
/****************************** update templates ******************************************/
update MetaTemplate
