...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-14379';
...
;with ContactHours as (	
    select MinLectureHour * ShortTermWeek as LectureHours	
    from CourseDescription	
    where CourseId = @entityId	
    union all	
    select MaxLectureHour * ShortTermWeek as LectureHours	
...
;with ContactHours as (
	select MinLabHour * ShortTermWeek as LabHours
	from CourseDescription	
	where CourseId = @entityId	
	union all	
	select MaxLabHour * ShortTermWeek as LabHours
	from CourseDescription	where CourseId = @entityId)
