...
DECLARE @JiraTicketNumber nvarchar(20) = 'IMP-16246';
DECLARE @Comments nvarchar(Max) = 
	'Fix Context Review tab to consume @FilterValue';
...
values
('Content Review', 'CourseContentReview', 'CourseContent','Update'),
('Content Review', 'CourseContentReviewRequisiteCourseObjective', 'CourseObjectiveId','Update2')
...
cr.Id as FilterValue 
	...
	WHERE cr.Id = @filterValue
	ORDER BY co.ObjectiveTypeId, co.Text
"

DECLARE @RSQL NVARCHAR(MAX) = "
Select Coalesce(co.[Text],co.GroupHeading) As Text from CourseObjective co  where co.Id = @Id
"
...
VALUES
(@MAX, 'CourseContentReviewRequisiteCourseObjective', 'Id', 'Title', @CSQL, @RSQL, 'Order By SortOrder', 'Consume Context Id for maverick', 3)
...
SELECT 'FilterSubscriptionTable', 'CourseContentReview', SectionId FROM @Fields WHERE Action = 'Update2'
UNION
SELECT 'FilterSubscriptionColumn', 'CourseRequisiteId', SectionId FROM @Fields WHERE Action = 'Update2'
/****************************** update templates ******************************************/
update MetaTemplate
