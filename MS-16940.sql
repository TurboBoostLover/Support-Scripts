...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-16940';
DECLARE @Comments nvarchar(Max) = 
	'Add Chained TOP code and CIP  code to program forms';
...
WHERE mt.Active = 1 
    ...
    AND mtt.ClientId = @clientId
		AND mtt.MetaTemplateTypeId in (490, 497)		--comment back in if just doing some of the mtt's
...
values
('Codes and Notes', 'ProgramCBCode', 'CB03Id','TOP'),
('Codes and Notes', 'Program', 'Accreditation','Note')
...
WHERE MetaSelectedSectionId in (
	SELECT TabId FROM @Fields WHERE Action = 'TOP'
)

UPDATE MetaSelectedField
SET RowPosition = RowPosition + 1
WHERE MetaSelectedFieldId in (
	SELECT FieldId FROM @Fields WHERE Action = 'Note'
)
...
SELECT 
ccs. id as Value, 
cast(Format(Cast( ccs.Code as Decimal (7,4)),'00.0000')AS NVARCHAR(15)) + ' - ' + Coalesce(cor.Title,ccs.Title) as Text,
ccs.SortOrder,
map.CB03Id AS FilterValue,
map.CB03Id AS filterValue
...
Select cc.Id as Value, cast(Format(Cast( cc.Code as Decimal (7,4)),'00.0000')AS NVARCHAR(15)) + ' - ' + Coalesce(cor.Title,cc.Title) as Text
from CipCode_Seeded cc
	left Join CipCode_Seeded_ClientOverride cor on cor.CipCode_SeededId = cc.Id
		and cor.Active = 1 
where cc.Id = @Id
"
...
NULL-- [Config]
FROM @Fields WHERE Action = 'TOP'

insert into MetaSelectedFieldAttribute
(Name,[Value],MetaSelectedFieldId)
SELECT 'FilterSubscriptionTable','ProgramCBCode',Id FROM @Fields2 UNION
SELECT 'FilterSubscriptionColumn','CB03Id',Id FROM @Fields2 UNION
SELECT 'FilterTargetTable','ProgramSeededlookup',Id FROM @Fields2 UNION
SELECT 'FilterTargetColumn','CipCode_SeededId',Id FROM @Fields2 UNION
/****************************** update templates ******************************************/
update MetaTemplate
