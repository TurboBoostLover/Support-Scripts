...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-19048';
DECLARE @Comments nvarchar(Max) = 
	'Adding Supplies to the COR report';
...
values
('4. REQUIRED TEXTS AND SUPPLIES:', 'CourseQueryText', 'QueryText_14','1')
...
SELECT 0 AS Value, CONCAT(CASE WHEN c.RequiredFeeText IS NOT NULL THEN concat('Additional instructional materials fees: ', c.RequiredFeeText, '<br>') ELSE '' END,CASE WHEN COUNT(ces.ID) > 0 THEN CONCAT('<ol><li>',
dbo.ConcatWithSepOrdered_Agg('<li>', SortOrder, OptionalText),'</ol>')
ELSE ''
END) AS Text
FROM Course aS c
LEFT JOIN CourseExitSkill AS ces on ces.CourseId = c.Id
WHERE C.Id = @EntityId
group by c.RequiredFeeText
"
...
VALUES
(@MAX, 'CourseExitSkill', 'Id', 'Title', @SQL, @SQL, 'Order By SortOrder', 'Supplies Query for COR', 2)

insert into [MetaSelectedField]
([DisplayName], [MetaAvailableFieldId], [MetaSelectedSectionId], [IsRequired], [MinCharacters], [MaxCharacters], [RowPosition], [ColPosition], [ColSpan], [DefaultDisplayType], [MetaPresentationTypeId], [Width], [WidthUnit], [Height], [HeightUnit], [AllowLabelWrap], [LabelHAlign], [LabelVAlign], [LabelStyleId], [LabelVisible], [FieldStyle], [EditDisplayOnly], [GroupName], [GroupNameDisplay], [FieldTypeId], [ValidationRuleId], [LiteralValue], [ReadOnly], [AllowCopy], [Precision], [MetaForeignKeyLookupSourceId], [MetadataAttributeMapId], [EditMapId], [NumericDataLength], [Config])
VALUES
(
'Supplies', -- [DisplayName]
8954, -- [MetaAvailableFieldId]
