...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-17418';
...
WHERE mt.Active = 1 
    ...
    AND mtt.ClientId = @clientId
		AND mtt.MetaTemplateTypeId in (546, 496, 497, 516)		--comment back in if just doing some of the mtt's
...
values
('Program Description', 'ProgramProposal', 'SemesterId','Ping')
...
DECLARE @Fields2 TABLE (FieldId int, SecId int, nam NVARCHAR(MAX))

insert into [MetaSelectedField]
([DisplayName], [MetaAvailableFieldId], [MetaSelectedSectionId], [IsRequired], [MinCharacters], [MaxCharacters], [RowPosition], [ColPosition], [ColSpan], [DefaultDisplayType], [MetaPresentationTypeId], [Width], [WidthUnit], [Height], [HeightUnit], [AllowLabelWrap], [LabelHAlign], [LabelVAlign], [LabelStyleId], [LabelVisible], [FieldStyle], [EditDisplayOnly], [GroupName], [GroupNameDisplay], [FieldTypeId], [ValidationRuleId], [LiteralValue], [ReadOnly], [AllowCopy], [Precision], [MetaForeignKeyLookupSourceId], [MetadataAttributeMapId], [EditMapId], [NumericDataLength], [Config])
output inserted.MetaSelectedFieldId, inserted.MetaSelectedSectionId, inserted.DisplayName INTO @Fields2
SELECT
'Are there any needed Library Resources?', -- [DisplayName]
...
([DisplayName], [MetaAvailableFieldId], [MetaSelectedSectionId], [IsRequired], [MinCharacters], [MaxCharacters], [RowPosition], [ColPosition], [ColSpan], [DefaultDisplayType], [MetaPresentationTypeId], [Width], [WidthUnit], [Height], [HeightUnit], [AllowLabelWrap], [LabelHAlign], [LabelVAlign], [LabelStyleId], [LabelVisible], [FieldStyle], [EditDisplayOnly], [GroupName], [GroupNameDisplay], [FieldTypeId], [ValidationRuleId], [LiteralValue], [ReadOnly], [AllowCopy], [Precision], [MetaForeignKeyLookupSourceId], [MetadataAttributeMapId], [EditMapId], [NumericDataLength], [Config])
SELECT
'Specify the type of resource(s) or why/what is specifically needed', -- [DisplayName]
...
SELECT s.TempId, f2.FieldId, s.SecId FROM @Sections AS s
INNER JOIN MetaSelectedSection AS mss on mss.MetaSelectedSectionId = s.SecId
INNER JOIN MetaTemplate AS mt on mss.MetaTemplateId = mt.MetaTemplateId
INNER JOIN MetaSelectedSection As mss2 on mss2.MetaTemplateId = mt.MetaTemplateId
INNER JOIN @Fields2 AS f2 on f2.SecId = mss2.MetaSelectedSectionId

while exists(select top 1 1 from @SHOWHIDE)
begin
	declare @Template int = (SELECT TOP 1 TId FROM @SHOWHIDE)
	DECLARE @Trigger int = (SELECT TOP 1 field FROM @SHOWHIDE)
	DECLARE @Sec int = (SELECT TOP 1 sec FROM @SHOWHIDE)
	...
	DECLARE @DisplayRuleName NVARCHAR(50) = 'Show/hide Resources';    
	DECLARE @SubscriberName NVARCHAR(50) = 'Show/hide Resources';    
...
exec EntityExpand @clientId =57 , @entityTypeId =2

exec spActivateWorkflow @processId = 447, @processVersionId = 914
exec spActivateWorkflow @processId = 431, @processVersionId = 915
exec spActivateWorkflow @processId = 446, @processVersionId = 916
exec spActivateWorkflow @processId = 432, @processVersionId = 917
exec spActivateWorkflow @processId = 433, @processVersionId = 918
exec spActivateWorkflow @processId = 430, @processVersionId = 919

INSERT INTO WorkflowCondition
(Title, Description, ConditionSQL, CommandTypeId, Active)
VALUES
('Library Resources', 'Only add Library Resources when the checkbox on the first tab is checked', '			--1 Not Included
			--2 Required
			--3 Optional

			select
				case WHEN ISNULL(gb.Bit01,0) = 1 THEN 2
					else 1
				end as StepTypeId
			from ProcessLevelActionHistory plah
				INNER join Program p on p.ProposalId = plah.ProposalId
				INNER JOIN GenericBit AS gb on gb.ProgramId = p.Id
			where plah.Id = @ProcessLevelActionHistoryId;', 1, 1)

DECLARE @New int = SCOPE_IDENTITY()

UPDATE Step
SET WorkflowConditionId = @New
WHERE Id in (9279, 9302, 9306, 9330)
