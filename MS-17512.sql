...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-17512';
DECLARE @Comments nvarchar(Max) = 
	'Update Urgent Request - Staff on Instructional Program Review';
...
values
('Urgent Request - Staff', 'ModuleYesNo', 'YesNo29Id','1'),
(NULL, 'GenericOrderedList01', 'ItemTypeId', '2'),
(NULL, 'GenericOrderedList01Lookup14', 'Rationale', '3'),
(NULL, 'GenericOrderedList01', 'Bit_01', '4'),
('Urgent Request - Staff', 'ModuleYesNo', 'YesNo03Id', '5'),
(NULL, 'GenericOrderedList01', 'MaxText01', '6'),
(NULL, 'GenericOrderedList01', 'YesNo01Id', '7')
...
SET SectionName = 'Request - Staff'
WHERE MetaSelectedSectionId in (
	SELECT TabId FROM @Fields WHERE Action = '1'
)

UPDATE MetaSelectedField
SET DisplayName = 'Do you have any STAFF requests?'
...
VALUES
(@MAX, 'Lookup01', 'Id', 'Title', @CSQL, @RSQL, 'Order By SortOrder', 'Instructional Lookup01', 1),
...
WHERE MetaSelectedFieldId in (
	...
	WHERE f.Action = '3'
	and msf.DisplayName like '%Lookup%'
	and f.SectionId <> 7236
)

DELETE FROM MetaSelectedField
WHERE MetaSelectedFieldId in (
	SELECT FieldId FROM @Fields WHERE Action = '3' and SectionId <> 7234
)
...
WHERE MetaSelectedFieldId in (
	SELECT FieldId FROM @Fields WHERE Action in ('6', '7')
)
...
DECLARE @Trigger int = (SELECT FieldId FROM @Fields WHERE ACtion = '4')
DECLARE @Listener int = (SELECT FieldId FROM @Fields WHERE Action = '6')
