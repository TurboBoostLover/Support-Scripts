...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-16349';
DECLARE @Comments nvarchar(Max) = 
	'Update Program Forms';
...
values
('Transfer Documentation', 'Program', 'IsGrad','Update1'),
('Transfer Documentation', 'Program', 'IsNewFeeRequired','Update2'),
('Transfer Documentation', 'Program', 'IsMandated','Update3'),
('Transfer Documentation', 'GenericOrderedList01', 'ItemTypeId','Delete')
...
SET DisplayName = '<a href="https://www.cccco.edu/About-Us/Chancellors-Office/Divisions/Educational-Services-and-Support/What-we-do/Curriculum-and-Instruction-Unit/Templates-For-Approved-Transfer-Model-Curriculum" target="_blank">CCCCO TMC Submission form</a> is completed and attached'
WHERE MetaSelectedFieldId in (
	SELECT FieldId FROM @Fields WHERE Action = 'Update1'
)

UPDATE MetaSelectedField
SET DisplayName = 'CSU/UC Baccalaureate Level Course List by Deparement is attached'
WHERE MetaSelectedFieldId in (
	SELECT FieldId FROM @Fields WHERE Action = 'Update2'
)

UPDATE MetaSelectedField
SET DisplayName = 'Articulation Agreement by Major (AAM) is attached'
WHERE MetaSelectedFieldId in (
...
SELECT DISTINCT
'Cal-GETC Certification Course List by Area (GECC) is attached', -- [DisplayName]
2233, -- [MetaAvailableFieldId]
