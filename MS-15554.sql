...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-15554';
...
DECLARE @reportResults TABLE 
	(
	  [Program Code] NVARCHAR(MAX)
	, [Program Title] NVARCHAR(MAX)
	, [Program Status] NVARCHAR(MAX)
	...
	, [Program of Study Reports] NVARCHAR(MAX)
	, [Sort Order] INT
	)
...
SELECT
	...
	, CAST (dbo.stripHtml (p.Rationale) AS NVARCHAR(MAX)) AS [Rationale] -- Get rid of HTML tags in output
    , CONCAT (''https://stpetersburg.curriqunet.com/DynamicReports/AllFieldsReportByEntity/'', p.Id, ''?entityType=Program&reportId=218'') 
	AS [Program of Study Reports] -- Link to Program of Study report for program proposal
	, sem.SortOrder
FROM Program p
	...
	INNER JOIN ClientEntitySubType cest ON pt.ClientEntitySubTypeId = cest.Id
WHERE p.Active = 1
	AND ClientEntitySubTypeId in (2, 3) -- Programs only; no Articulations or Transfer Plans MS-15554 added transfer plans back 
	AND (@useAllTerms = 1 OR sem.Id IN (SELECT Value FROM @terms)) -- Either use all the terms or the selected one(s)
...
SELECT
	...
	, [Rationale] 
	, [Program of Study Reports] 
FROM @reportResults
ORDER BY [Program Code], [Program Title], [Sort Order] ASC; 
'
WHERE Id = 34