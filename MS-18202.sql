...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-18202';
DECLARE @Comments nvarchar(Max) = 
	'Add validation to require a SLO mapped to ILO';
...
values
('Student Learning Outcomes/Objectives', 'CourseOutcome', 'OutcomeText','1')
...
DECLARE @Valid int = (SELECT Count(co.Id) FROM CourseOutcome AS co WHERE co.CourseId = @EntityId and co.ID in (SELECT CourseOutcomeId FROM CourseOutcomeClientLearningOutcome) and co.OutcomeText IS NOT NULL)
...
SELECT SectionId, 'SLO', 'Require map to ILO', 6, 'Ensure the Outcome Text is filled out and at least one ILO is checked.', @Id FROM @Fields
