...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-18624';
DECLARE @Comments nvarchar(Max) = 
	'Update to Instrutional PSR Annual Update';
...
values
('ONGOING Budget Augmentation Requests', 'GenericOrderedList02', 'Decimal01','1'),
('Funding For Repair/Replacement Requests', 'GenericOrderedList04', 'Decimal01', '2')
...
where Active = 1
and Title like ''Fall%''
and AcademicYearStart between year(getDate()) - 1 and year(getDate()) + 4
union
...
from Module m
inner join Semester sem on m.SemesterId = sem.Id
where m.Id = @entityId
order by [Text]
		'
WHERE Id = 269
...
UPDATE ListItemType
SET ListItemTitleColumn = 'MaxText03'
WHERE Id = 15
...
DECLARE @count INT = (
SELECT	Count(Decimal01) + COUNT(MaxText01) + COUNT(MaxText02) + COUNT(MaxText03)
FROM GenericOrderedList02
WHERE ModuleId = @entityId
);
