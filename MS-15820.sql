...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-15820';
...
values
('Section II', 'CourseEvaluationMethod', 'EvaluationMethodId','Update')
...
SELECT CONCAT('<ol><li>', dbo.ConcatWithSepOrdered_Agg('<li>', em.SortOrder, CONCAT(em.Title, '</li>')),
'</ol>', c.AdvisoryCommittee
) AS Text FROM CourseEvaluationMethod AS cem
INNER JOIN EvaluationMethod AS em on cem.EvaluationMethodId = em.Id
INNER JOIN Course AS c on cem.CourseId = c.Id
WHERE CourseId = @EntityId
group by c.AdvisoryCommittee
"
...
VALUES
(@MAX, 'CourseEvaluationMethod', 'Id', 'Title', @CSQL, @RSQL, 'Order By SortOrder', 'Look up for COR report', 2)
