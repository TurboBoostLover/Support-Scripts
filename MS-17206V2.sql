...
select
	...
	concat(dbo.fnHtmlOpenTag(@blockTag, dbo.fnHtmlAttribute(@classAttrib, ''row'')),
				case when len(ed.ExtraDetailsIcons) > 0 then concat(
					@space,
					dbo.fnHtmlOpenTag(@inlineTag, dbo.fnHtmlAttribute(@classAttrib, ''course-entry-icons'')),
						ed.ExtraDetailsIcons,
			...
			dbo.fnHtmlOpenTag(@blockTag, dbo.fnHtmlAttribute(@classAttrib, ''col-xs-4 col-sm-4 col-md-4'')),
				dbo.fnHtmlOpenTag(@inlineTag, dbo.fnHtmlAttribute(@classAttrib, ''course-Units'')),
					format(PC.CalcMin,''0.0#''),
					case when PC.CalcMin <> PC.CalcMax then '' - '' + format(PC.CalcMax,''0.0#'') end,
				dbo.fnHtmlCloseTag(@inlineTag),
			...
			dbo.fnHtmlOpenTag(@blockTag, dbo.fnHtmlAttribute(@classAttrib, ''col-xs-4 col-sm-4 col-md-4'')),
				...
				dbo.fnHtmlCloseTag(@inlineTag),
			dbo.fnHtmlCloseTag(@blockTag),
			dbo.fnHtmlOpenTag(@blockTag, dbo.fnHtmlAttribute(@classAttrib, ''col-xs-4 col-sm-4 col-md-4 text-end'')),
				dbo.fnHtmlOpenTag(@inlineTag, dbo.fnHtmlAttribute(@classAttrib, ''course-Semester'')),
					case when len(Sem.txt) > 0 then left(Sem.txt,len(Sem.txt) - 1) else ''&nbsp;'' end,
				dbo.fnHtmlCloseTag(@inlineTag),
			dbo.fnHtmlCloseTag(@blockTag),
...
VALUES
(''NonCourseRow'', ''LeftColumn'', ''col-xs-6 col-sm-6 col-md-6 two-column left-column text-left text-start''),
(''NonCourseRow'', ''RightColumn'', ''col-xs-6 col-sm-6 col-md-6 two-column right-column text-left text-start'')
...
UPDATE MetaReport
SET ReportAttributes = '{"reportTemplateId":7,"cssOverride":".col-xs-2.col-sm-2.col-md-2.right-column.text-right{text-align: right;} @media print {.row{display: flex;}}"}'
WHERE Id = 475
