...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-14413';
DECLARE @Comments nvarchar(Max) = 
	'Create Custom Catalog queries to speed it up a little';
...
', 'Custom Course Summary', 'Should be like standard but faster', GETDATE())
...
(OutputModelClientId, OutputTemplateClientId, Title, Description)
VALUES
(@MOD, @TEMP, 'Course summary', 'Should pull the same information as the standard but faster')

DECLARE @TEST int = SCOPE_IDENTITY()

UPDATE CurriculumPresentationOutputFormat
SET OutputTemplateModelMappingBaseId = NULL
, OutputTemplateModelMappingClientId = @TEST
WHERE CurriculumPresentationId = 1