...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-15745';
DECLARE @Comments nvarchar(Max) = 
	'Update CIC Course Admin report';
...
--EXEC tempdb.dbo.sp_help @objname = N''#Proposals'';

declare @entityList dbo.Integers;
insert into @entityList 
select distinct [Course Id]
from #Proposals

declare @curriculumPresentationId int = (
	select Id
	from CurriculumPresentation
	where Title = ''Course Requisites Summary''
);

declare @additionalTemplateConfig nvarchar(max) = ''{"RenderV2Requisites": false}''
...
--insert into @results (CourseId, Requisites)
exec upRenderCurriculumPresentation 
	@curriculumPresentationId = @curriculumPresentationId,
	@outputFormatId = 5,
	@entityTypeId = 1,
	@entityList = @entityList,
	@additionalTemplateConfig = @additionalTemplateConfig,
	@resultTable = #results
...
, [Offered At]
, [Action(s) Proposed]
, [Proposed for College(s)]
, [Originating Campus]
, [Dist. Ed Proposed For College(s)]
, [Effective]
, [Proposed for: Credit for Prior Learning]
, [C-ID]
from #Proposals p
	inner join #results req on p.[Course Id] = req.[Value]
order by [Subject Code], dbo.fnCourseNumberToNumeric([Course Number]), [Course Number];
