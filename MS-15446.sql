...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-15446';
DECLARE @Comments nvarchar(Max) = 
	'Update CustomSQL in outcome outline report';
							...
							, case
								...
								and cd.MinLabLecHour > 0
									then
										cast(
											cast(
												(coalesce(cd.MinLabLecHour, 0) / 17.5)
											as decimal(16, 3))
										as nvarchar)
								else
									case
								...
								and cd.MaxLabLecHour > cd.MinLabLecHour
									then
										concat(
											...
											, cast(
												cast(
													(coalesce(cd.MaxLabLecHour, 0) / 17.5)
												as decimal(16, 3))
											as nvarchar)
										)
								else
...
WHERE MetaTemplateId in (
	...
	WHERE msf.MetaForeignKeyLookupSourceId = 4038
)