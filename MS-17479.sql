...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-17479';
DECLARE @Comments nvarchar(Max) = 
	'Add tab to courses for CCN';
...
/********************** Changes go HERE **************************************************/
DECLARE @Existing INTEGERS
INSERT INTO @Existing
SELECT MetaSelectedSectionId FROM MetaSelectedSection WHERE MetaBaseSchemaId = 81
...
'Common Course Numbering Template', -- [SectionName]
...
DECLARE @Id1 int = (SELECT ISNULL(Max(Id), 0) + 1 FROM AttachmentType)
DECLARE @Id2 int = (SELECT ISNULL(Max(Id), 0) + 2 FROM AttachmentType)

INSERT INTO AttachmentType
(Title, EntityTypeId, SortOrder, StartDate, Id)
VALUES
('Attachments Tab', 1, 1, GETDATE(), @Id1),
('Common Course Numbering', 1, 1, GETDATE(), @Id2)

UPDATE CourseAttachedFile
SET AttachmentTypeId = @Id1
WHERE AttachmentTypeId IS NULL

INSERT INTO MetaSelectedSectionAttribute
(Name, Value, MetaSelectedSectionId)
SELECT 'AttachmentType', @Id2, Id FROM @Sections
UNION
SELECT 'AttachmentType', @Id1, Id FROM @Existing
/****************************** update templates ******************************************/
update MetaTemplate
