...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-18497';
...
values
('Target Students From Other Schools', 'CourseSchool', 'OrganizationEntityId','1'),
('Target Students From Other Schools', 'CourseSchool', 'Bit01','2')
...
select DISTINCT
	ec.Id as Value,
	ec.Title as Text,
	map.DisciplineTypeId as FilterValue
from Course c
	inner join CourseSchool AS cs on cs.CourseId = c.Id
	inner join OrganizationEntityDisciplineTypeSubDivisionCategoryEligibilityCriteriaMap AS map on map.DisciplineTypeId = cs.DisciplineTypeId
		and map.OrganizationEntityId = cs.OrganizationEntityId
	inner join EligibilityCriteria AS ec on map.EligibilityCriteriaId = ec.Id
	WHERE ec.Active = 1
	and cs.Id = @ppkIdValue
"

DECLARE @RSQL NVARCHAR(MAX) = "
select Title as Text from EligibilityCriteria where Id = @id
"
...
VALUES
(@MAX, 'CourseSchoolEligibility', 'Id', 'Title', @CSQL, @RSQL, 'Order By SortOrder', 'Award stuff check list in nest ol', 3)
...
SELECT
'Eligibility Criteria', -- [DisplayName]
12773, -- [MetaAvailableFieldId]
...
SELECT 'helptext', 'If school is changed the Curriculum/Programme Code needs to be deselected then reselected.', FieldId FROM @Fields WHERE Action = '1'

INSERT INTO MetaSelectedSectionAttribute
(Name, Value, MetaSelectedSectionId)
SELECT 'lookuptablename', 'CourseSchoolEligibility', Id FROM @Sections
UNION
SELECT 'lookupcolumnname', 'EligibilityCriteriaId', Id FROM @Sections
UNION
SELECT 'columns', '1', Id FROM @Sections
UNION
SELECT 'FilterSubscriptionTable', 'CourseSchool', Id FROM @Sections
UNION
SELECT 'FilterSubscriptionColumn', 'DisciplineTypeId', Id FROM @Sections
...
INSERT INTO OrganizationEntityDisciplineTypeLookup14Map
(OrganizationEntityId, DisciplineTypeId, Lookup14Id, ClientId, StartDate)
VALUES
(50, 8, 124, 1, GETDATE()),
(50, 10, 124, 1, GETDATE()),
(50, 9, 124, 1, GETDATE())
