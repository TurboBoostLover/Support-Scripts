...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-14175';
DECLARE @Comments nvarchar(Max) = 
	'Update show hide on program form';
...
WHERE mt.Active = 1 
    ...
    AND mtt.ClientId = @clientId
		AND mtt.MetaTemplateTypeId not in (14, 15, 17)
...
values
('Cover', 'Program', 'ProgramTypeId','Update'),
('Transfer Documentation', 'Program', 'HasRequirements','Update2')
...
, Operand2Literal = 2
WHERE Id in (
SELECT ep.Id FROM ExpressionPart AS ep
...
INNER JOIN MetaDisplaySubscriber AS mds on mds.MetaDisplayRuleId = mdr.Id
	WHERE ep.Operand1_MetaSelectedFieldId in (
		SELECT FieldId FROM @Fields WHERE Action = 'Update'
		)
		AND mds.MetaSelectedSectionId in (
