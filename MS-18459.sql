...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-18459';
DECLARE @Comments nvarchar(Max) = 
	'Move the Semester Table inside the Major Table';
...
values
('School(s)', 'CourseSchoolSemester', 'SemesterId','1'),
('School(s)', 'CourseSchoolSemester', 'ItemTypeId','2'),
('School(s)', 'CourseSchoolMajor', 'Lookup14Id', '3'),
('School(s)', 'CourseSchoolEligibility', 'EligibilityCriteriaId', '4')
...
SET MetaSelectedSection_MetaSelectedSectionId = f2.SectionId
, SortOrder = 3
, RowPosition = 3
, MetaBaseSchemaId = 5710
, MetaSectionTypeId = 32
...
SET MetaAvailableFieldId = 10965
WHERE MetaSelectedFieldId in (  --3232)
	SELECT FieldId FROM @Fields WHERE Action = '1'
)

UPDATE MetaSelectedField
SET MetaAvailableFieldId = 10967
WHERE MetaSelectedFieldId in (  --3241)
	SELECT FieldId FROM @Fields WHERE Action = '2'
)

INSERT INTO MetaSelectedSectionAttribute
(Name, Value,MetaSelectedSectionId)
SELECT 'lookuptablename','CourseSchoolMajorSemester', SectionId FROM @Fields WHERE ACtion = '1'
UNION
SELECT 'lookupcolumnname','SemesterId', SectionId FROM @Fields WHERE ACtion = '1'
UNION
