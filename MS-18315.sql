...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-18315';
DECLARE @Comments nvarchar(Max) = 
	'Update Program Header on Catalog';
...
''SELECT
	...
	<div class="row summary-header">
		...
		<div class="col-xs-2 col-sm-2 col-md-2 text-right header-units">
			'''',CASE WHEN p.AwardTypeId in (Select Id 
					from AwardType 
					Where 
						(
							Title Like ''''%Certificate of Competency'''' 
								Or Title Like ''''%Certificate of Completion'''' 
								Or Title Like ''''%Certificate of Accomplishment''''
						)) THEN ''''HOURS''''
				ELSE ''''UNITS''''
			END,
		''''</div>
...
values (
	...
	"trigger": "focus",
	"content": "",
	"curriculumPresentationId": '' + cast(@courseCurriculumPresentationId as nvarchar) + ''}''''
);
