...
set @sql = 
'
DECLARE @type bit = (SELECT CASE WHEN  p.AwardTypeId = 1 THEN 28 ELSE 0 END FROM Program AS p WHERE p.Id = @EntityId)
	...
	BEGIN
		...
		WHILE(@counter2 < @TotalSpecialisations)
			BEGIN
				...
				declare @Warningmessage NVARCHAR(max) = (select case 
					when not exists(select top 1 1 from @CurrentCoursesCalculations where QFL = ''QFLevel 5'') 
...
update MetaTemplate set LastUpdatedDate = GETDATE() where MetaTemplateId = 25