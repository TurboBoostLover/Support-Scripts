...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-17829';
DECLARE @Comments nvarchar(Max) = 
	'Update Fields to have custom SQL to not show duplicates';
...
DECLARE @TABLE TABLE (ID int, txt NVARCHAR(MAX))
INSERT INTO @TABLE
exec spSubjectLookupByUserPermission @clientId, @userId, @entityId,1 /*Course*/

SELECT DISTINCT ID AS Value, txt AS Text FROM @TABLE ORDER BY txt
"

DECLARE @RSQL NVARCHAR(MAX) = "
SELECT CONCAT('(', SubjectCode,') ', Title) AS Text FROM Subject WHERE Id = @Id
"
...
VALUES
(@MAX, 'Subject', 'Id', 'Title', @CSQL, @RSQL, 'Order By SortOrder', 'Subject filtered by origination rights and inner joined to course.', 2)
