...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-14969';
DECLARE @Comments nvarchar(Max) = 
	'Update MetaTemplate Titles and PR years';
...
WHERE mt.Active = 1 
    ...
    AND mtt.ClientId = @clientId
		AND mtt.MetaTemplateTypeId in (530)		--comment back in if just doing some of the mtt's
...
values
('Main', 'ModuleDetail', 'Tier3_OrganizationEntityId','Ping')
...
SELECT
    Id AS Value,
    CAST(Title AS nvarchar(max)) AS Text
FROM YearLookup
WHERE Id >= YEAR(GETDATE()) - 4 AND Id < YEAR(GETDATE())
'
WHERE Id = 68

INSERT INTO MetaTitleFields
(MetaTemplateId, MetaSelectedFieldId, Ordinal)
SELECT TemplateId, FieldId, 1 FROM @Fields

UPDATE MetaTitleFields
SET Ordinal = 2
WHERE Id = 193
