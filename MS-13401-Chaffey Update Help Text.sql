...
DECLARE @JiraTicketNumber nvarchar(20) = 'MS-13401';
DECLARE @Comments nvarchar(Max) = 
	'Update the help text on program deactivations tab';
...
values
('General Program Information', 'ProgramProposal', 'ArticulateCourse','Remove'),
('General Program Information', 'Program', 'ChangeRationale', 'Find')
...
ON MSS.MetaSelectedSection_MetaSelectedSectionId = MSS2.MetaSelectedSection_MetaSelectedSectionId
WHERE MSS.SortOrder = 0
AND MSS2.MetaSelectedSectionId IN (SELECT SectionId FROM @Fields WHERE Action = 'FIND')

Update MetaSelectedField
Set DisplayName = '<b class= "text-success"> To complete this deactivation proposal, simply provide the rationale for the deactivation in the space below and launch!</b>'
WHERE MetaSelectedSectionId IN (SELECT MS FROM @RealId)
